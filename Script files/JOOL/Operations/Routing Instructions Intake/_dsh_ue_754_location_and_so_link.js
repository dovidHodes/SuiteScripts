        ...
        try {
            ...
            if (shipPointValue) {
                ...
                try {
                    ...
                    // Field is not searchable, so we load all locations and check the field value
                    var locationResults = [];
                    
                    var allLocationsSearch = search.create({
                    ...
                    allLocationsSearch.run().each(function(result) {
                        try {
                            ...
                            var locShipPoint = locRecord.getValue('custrecord_wm_ship_point');
                            if (locShipPoint == shipPointValue) {
                                locationResults.push(result.id);
                            }
                        } catch (loadError) {
                            // Skip this location if we can't load it
                            log.debug('beforeSubmit', 'Could not load location ' + result.id + ': ' + loadError.toString());
                        }
                        return true;
                    });
                    ...
                    log.error('beforeSubmit', 'Error searching for locations: ' + JSON.stringify(locationSearchError));
                    log.error('beforeSubmit', 'Error details - name: ' + (locationSearchError.name || 'N/A') + ', message: ' + (locationSearchError.message || 'N/A'));
                    var existingIssue = newRecord.getValue('custbody_issue') || '';
            ...
            if (!entityId) {
                ...
                // Check if line count is valid (should be >= 0)
                if (lineCount < 0) {
                    log.audit('beforeSubmit', 'Invalid line count (' + lineCount + ') - sublist "line" may not exist or be accessible. Skipping line processing.');
                } else {
