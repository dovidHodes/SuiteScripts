...
// BEGIN SCRIPT DESCRIPTION BLOCK ==================================
{
  ...
  -- version--   -- Date --   -- Modified By --          --Requested By--                       -- Description --
       ...
       4.0       04-09-2024    Aviva Wilen      Yoel Gras     
       */
}
  ...
  function mainForm(context, parameters) {
    try {
      var form = serverWidget.createForm({
        title: "Consolidated BOL 2.0",
        subtitle: "Enter the Load ID and press Submit 2 times"
      });
    ...
    //    custpage_startdate: parameters.custpage_startdate,
     ...
     //   custpage_customer: parameters.custpage_customer,
        custpage_loadid: parameters.custpage_loadid,
...
//log.debug('postMethod',parameters);
    ...
    parameters.custpage_array_of_tabs) {
      ...
      log.audit('her is msat and ind', parameters)
      getSublistDataPDF(parameters, context, true);
      getSublistDataPDF(parameters, context, false);
...
if(radioField1Value=='standalone'){
  ...
  function openPdfInTab(context, consolidated, parameters) {
    ...
    log.audit('output url', outputUrl)
    let pdfArr = [];

    var pdfUrlSearch = pdfUrlSearchFunc(consolidated);
    log.audit('pdfUrlSearch 232',pdfUrlSearch)
    ...
    for (let i = 0; i < pdfUrlSearch.length; i++) {
      ...
      log.audit('pdfUrlSearch 245',pdfUrl)
  ...
  function masterIndvPdfUrlArr(consolidated, pdfUrlSearch, pdfArr) {
     log.audit ('pdfArr311 in masterIndvPdfUrlArr',pdfArr)
   // pdfArr.push(pdfArr.shift())
   pdfArr.unshift(pdfArr.pop())
   // log.debug('pdfArr322',pdfArr)
         log.audit('pdfArr311 in masterIndvPdfUrlArr afterPop',pdfArr)
  ...
  function makeSuiteLetUrl(parameters) {
    var suiteletURL = url.resolveScript({
      scriptId: "customscript_gbs_aw_consolidated_bol",
      deploymentId: "customdeploy_gbs_aw_consolidated_bol",
      params: {
   //     custpage_startdate: parameters.custpage_startdate,
    //    custpage_enddate: parameters.custpage_enddate,
     //   custpage_customer: parameters.custpage_customer,
  ...
  function getSublistDataPDF(parameters, context, consolidated) {
   ...
   //     }
      ...
      var dateFormat = ''+currentMonth+today.getDate()+today.getFullYear();
      masterData.body.mblNumber=parameters.custpage_loadid+'-'+dateFormat;

      if(parameters.custpage_radiofield=='masterandinvd'){
       masterData.body.mabdDateCommon=getMAdbDate(ItemFullIdArr)
       }
      masterData.body.scac = getScac(ItemFullIdArr);
      masterData.body.nmfcCalc =parameters.custpage_nmfc;
      masterData.body.nmfc = getNfmc(ItemFullIdArr);
      masterData.body.bclass = getClass(ItemFullIdArr);
...
var mabdField = form.addField({
  ...
  function getTransactions(recType, loadid) {
 ...
 sublist.displayType = serverWidget.SublistDisplayType.HIDDEN;
  ...
  log.debug(results[[0]]);
   return results[0].getValue({
    ...
    function getClass(ifIdArr){
    log.debug('getClass',ifIdArr);
    var results = search
    .create({
      type: 'itemfulfillment',
      filters: [
     ...
     //   'AND',
        ['internalid', 'anyof', ifIdArr],
         "AND", 
        ["item.custitemdensity","isnotempty",""]
      ],
       columns: [
           search.createColumn({
            name: "quantity",
 ...
 // log.debug('nmfc results',results)
  if (!results || results.length == 0) {
    log.debug('no results density')
    return false
  }
  log.debug(results[[0]]);
    var density = results[0].getValue({
            name: "custitemdensity",
            join: "item",
            label: "Density",
            summary: search.Summary.GROUP
    ...
    if (density <1) {var bclass = 400} else
    if (density <2) {var bclass = 300} else
    if (density <4) {var bclass = 250} else
    if (density <6) {var bclass = 175} else
    if (density <8) {var bclass = 125} else
    if (density <10) {var bclass = 100} else
    if (density <12) {var bclass = 92.5} else
    if (density <15) {var bclass = 85} else
    if (density <22.5) {var bclass = 70} else
    if (density <30) {var bclass = 65} else
    {var bclass = 60}
    return bclass
  }
  function getScac(ifIdArr){
   // log.debug('ifIdArr',ifIdArr)
    var results = search
    .create({
      type: 'itemfulfillment',
      filters: [
        ...
        ['internalid', 'anyof', ifIdArr],
        "AND", 
        ["custbody_sps_carrieralphacode","isnotempty",""],
        "AND", 
        ["custbody_sps_carrieralphacode","isnot","F"]
      ],
       columns: [
        search.createColumn({name: "ordertype", label: "Order Type"}),
        search.createColumn({
  ...
  if (!results || results.length == 0) {
   ...
   return results[0].getValue({ name: "custbody_sps_carrieralphacode",
   sort: search.Sort.ASC,
   label: "Standard Carrier Alpha Code"})
  }
