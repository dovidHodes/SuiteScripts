 ...
 * 
 * Custom version of SPS MR autopack script with enhanced debugging
 * Based on sps_mr_auto_pack_2x.js
 * 
 * NOTE: Library paths point to SPS Scripts folder - adjust if needed
 */
define(["require", "exports", "N/error", "N/log", "N/runtime", "N/record", "../SPS Scripts/Autopack/lib/sps_lib_script_status_rec", "../SPS Scripts/Autopack/lib/sps_lib_auto_pack_v2", "../SPS Scripts/Autopack/lib/sps_lib_auto_pack", "../SPS Scripts/Autopack/lib/sps_lib_script_status_rec_refactor"], function (require, exports, error, log, runtime, record, sps_lib_script_status_rec_1, sps_lib_auto_pack_v2_1, sps_lib_auto_pack_1, sps_lib_script_status_rec_refactor_1) {
    
    // ============================================
    // CUSTOM FIELD PLACEHOLDERS - Specify fields here
    // ============================================
    // Package Record Custom Fields (customrecord_sps_package)
    var PACKAGE_CUSTOM_FIELD_1 = null; // TODO: Specify field ID (e.g., 'custrecord_my_custom_field')
    var PACKAGE_CUSTOM_FIELD_2 = null; // TODO: Specify field ID if needed
    
    // Item Fulfillment Custom Fields
    var IF_CUSTOM_FIELD_1 = null; // TODO: Specify field ID (e.g., 'custbody_my_custom_field')
    var IF_CUSTOM_FIELD_2 = null; // TODO: Specify field ID if needed
    // ============================================
    ...
    function getInputData(inputContext) {
        log.audit('CUSTOM_DEBUG', '=== MR AUTOPACK SCRIPT STARTED ===');
        ...
        try {
            ...
            ifTranId = ifRecord.getValue('tranid') || ifId;
        } catch (e) {
            log.error('CUSTOM_DEBUG', 'Error loading IF for tranID: ' + e.toString());
        }
        
        log.audit('CUSTOM_DEBUG', 'MR called for IF: ' + ifTranId + ' (ID: ' + ifId + ')');
        log.debug('CUSTOM_DEBUG', 'Step 1: Initializing script status record');
        ...
        log.debug('CUSTOM_DEBUG', 'Step 2: Building IF line item object');
        ...
        log.audit('CUSTOM_DEBUG', 'IF ' + ifTranId + ' - Carton count: ' + itemFulfillmentLineItemObj.cartonCount);
        
        if (Object.keys(itemFulfillmentLineItemObj.lineItems).length > 0) {
            log.debug('CUSTOM_DEBUG', 'Step 3: Processing ' + Object.keys(itemFulfillmentLineItemObj.lineItems).length + ' line item(s)');
            
            var arrOfItemIds = Object.values(itemFulfillmentLineItemObj.lineItems).map(function (lineItem) {
                return lineItem.itemId;
            });
            
            log.debug('CUSTOM_DEBUG', 'Step 4: Getting packing rules for items');
            var ifPackedRules = sps_lib_auto_pack_1.getRulePackMappedObj(arrOfItemIds);
            
            var _loop_1 = function (lineItem) {
                log.debug('CUSTOM_DEBUG', 'Step 5: Processing line item ' + lineItem);
        ...
        log.debug('CUSTOM_DEBUG', 'Step 6: Building package array for MapReduce');
        ...
        log.audit('CUSTOM_DEBUG', 'IF ' + ifTranId + ' - Input stage complete. Starting to create ' + (fullOrderPackArr.length - 1) + ' package(s)');
    ...
    function reduce(reduceContext) {
        ...
        if (reduceKeyNum !== '00') {
            log.debug('CUSTOM_DEBUG', 'Step 7: Creating package with carton index ' + reduceVal['cartonIndex']);
            // do not try to pack first key as that is a value we use later
            var packageId = sps_lib_auto_pack_v2_1.createOneSpsPackageAndPackageContentRecsUsingSpsPackObj(reduceVal);
            
            // ============================================
            // CUSTOM: Write to Package Record (if packageId returned and fields specified)
            // ============================================
            if (packageId && (PACKAGE_CUSTOM_FIELD_1 || PACKAGE_CUSTOM_FIELD_2)) {
                try {
                    ...
                    // TODO: Set your custom field values here
                    // Example:
                    // if (PACKAGE_CUSTOM_FIELD_1) {
                    //     packageRecord.setValue({
                    //         fieldId: PACKAGE_CUSTOM_FIELD_1,
                    ...
                    log.debug('CUSTOM_DEBUG', 'Updated custom fields on package ' + packageId);
                } catch (e) {
                    log.error('CUSTOM_DEBUG', 'Error updating package ' + packageId + ' custom fields: ' + e.toString());
                }
            }
            // ============================================
        }
    ...
    function summarize(summaryContext) {
        log.audit('CUSTOM_DEBUG', '=== SUMMARIZE STAGE STARTED ===');
        ...
        var ifTranId = itemFulObj.transactionNumber || itemFulObj.itemfulfillmentRecordId || 'Unknown';
        
        log.debug('CUSTOM_DEBUG', 'Step 8: Processing errors and updating status');
        ...
        summaryContext.reduceSummary.errors.iterator().each(function (key, error, executionNo) {
            ...
            log.error('CUSTOM_DEBUG', 'Package creation error: ' + JSON.stringify(error));
            errorCount += 1;
            return true;
        });
        ...
        log.debug('CUSTOM_DEBUG', 'Step 9: Updating script status record');
        ...
        log.debug('CUSTOM_DEBUG', 'Step 10: Updating IF packed quantities and carton count');
        ...
        // ============================================
        // CUSTOM: Write to Item Fulfillment Record (if fields specified)
        // ============================================
        if (IF_CUSTOM_FIELD_1 || IF_CUSTOM_FIELD_2) {
            try {
                var ifRecord = record.load({
                    type: record.Type.ITEM_FULFILLMENT,
                    id: itemFulObj.itemfulfillmentRecordId,
                    isDynamic: true
                });
                
                // TODO: Set your custom field values here
                // Example:
                // if (IF_CUSTOM_FIELD_1) {
                //     ifRecord.setValue({
                //         fieldId: IF_CUSTOM_FIELD_1,
                //         value: 'Your value here'
                //     });
                // }
                ...
                log.debug('CUSTOM_DEBUG', 'Updated custom fields on IF ' + ifTranId);
            } catch (e) {
                log.error('CUSTOM_DEBUG', 'Error updating IF ' + ifTranId + ' custom fields: ' + e.toString());
            }
        }
        // ============================================
        
        alertScriptUsage(); //checking script usage for logging
        log.audit('CUSTOM_DEBUG', '=== MR AUTOPACK COMPLETE FOR IF: ' + ifTranId + ' ===');
