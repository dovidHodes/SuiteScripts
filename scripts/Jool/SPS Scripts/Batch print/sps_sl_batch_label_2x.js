 ...
 */
define(["require", "exports", "N/error", "N/log", "N/config", "N/record", "N/task", "./lib/sps_lib_packdata", "./lib/sps_lib_create_if_obj", "./lib/sps_lib_label", "./lib/sps_lib_label_api", "./lib/sps_lib_label_removal"], function (require, exports, error, log, config, record, task, packdata, createIfObj, label, spsapi, removeLabel) {
    ...
    function onRequest(ctx) {
        ...
        if (paramObj.rowLimitOverride) {
            ...
            if (rowLimitInt > 0) {
                ...
                log.debug('Manual Row Limit Override', "New row limit param passed with a limit of " + rowLimit);
            }
        }
        ...
        if (typeof itemFulfillmentStr === 'string') {
            ...
            if (itemFulfillmentArr.length > 0) {
                ...
                var packageObj = packdata.getArray(itemFulfillmentArr, packageId, packageSource, packStructure);
                ...
                if (labelCount > rowLimit) {
                    var taskId = '';
                    for (var deploymentCounter = 0; deploymentCounter < NUM_DEPLOYMENTS; deploymentCounter++) {
                        try {
                            var mapReduceTask = task.create({
                                taskType: task.TaskType.MAP_REDUCE,
                                scriptId: 'customscript_sps_mr_batch_label',
                                deploymentId: "customdeploy_sps_mr_batch_label_" + deploymentCounter,
                                params: {
                    ...
                    if (taskId) {
                        record.submitFields({
                            ...
                            values: {
                                custbody_sps_lbl_msg: 'Label Generation passed to Map Reduce script',
                            },
                            options: {
                        ...
                        ctx.response.write('Creation of Shipping Labels has been scheduled. Please see Map Reduce Script Status for notice of completion.');
                        log.debug('Map Reduce Handoff', 'Creation of Shipping Labels has been scheduled. Please see Map Reduce Script Status for notice of completion.');
                    }
                    else {
                        ctx.response.write('Unable to schedule since all script deployments are in use for SPS MR Batch Labels. Please see Map Reduce Script Status and retry when a deployment is available for SPS MR Batch Labels.');
                    }
                    return;
                }
                ...
                var itemFulfillmentObj = createIfObj.getIfObj(itemFulfillmentArr, labelSearchId, packageSource);
                var itemFulfillmentResults = itemFulfillmentObj.ifResults;
                log.debug('ifObj', JSON.stringify(itemFulfillmentResults));
                var results = label.getLabelResultObj(itemFulfillmentArr, itemFulfillmentResults, packageObj, compLabelSettings, maxLabelRequest);
                ...
                var labelUid_1 = itemFulfillmentResults[firstKey[0]].LabelUID || label.getDefaultLabelUID(itemFulfillmentArr[0]) || false;
                ...
                labelXmlArr.forEach(function (labelXml, index) {
                    ...
                    var futureFileName = label.spsBuildFileName(currIfId_1, currentLabelCount_1, labelBatchCount_1, packageId);
                    if (logDebugXmlFile) {
                        label.logXmlRecordForTesting(labelReqXML, futureFileName);
                    }
                    var labelObj;
                    try {
                        labelObj = spsapi.spsLabelApiRequest(labelReqXML, labelApiToken_1, labelUid_1, futureFileName);
                    }
                    catch (labelApiError) {
                        if (packageId) {
